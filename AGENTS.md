...
- Frontmatter contract: `title` (required), `date` (required), `status` (public|draft|private, required), and optional `slug`, `author`, `tags` (defaults to empty), `category`, `notebook` (at most one per note, drives the vault filter pills), `description`, `project` (renders on /projects), `stack` (project tech stack), `image` (project cover URL), `lang` (en|pt), `translation` (slug of the counterpart note).
